...
function connectDB() {
    try {
        ...
        // Only create tables if database is new or doesn't have tables
        if (!dbExists || !tableExists('users')) {
...
function seedData() {
    try {
        // Only seed if amenities table is empty
        const count = db.prepare("SELECT COUNT(*) as count FROM amenities").get();
        if (count.count === 0) {
            const amenities = [
